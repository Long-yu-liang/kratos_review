...
func (r *reviewRepo) ListReviewByStoreID(ctx context.Context, storeID int64, offset, limit int) ([]*biz.MyReviewInfo, error) {
	// 去ES里面查询评价  高并发查询时，直接查询ES不妥，通过redis中间件缓存，实现缓存
...
func (r *reviewRepo) getData2(ctx context.Context, storeID int64, offset, limit int) ([]*biz.MyReviewInfo, error) {
	//1、先查询Redis缓存
	//2、缓存没有，查询ES
	//3、通过singleflight模式，合并短时间内大量的并发查询
...
// getDataBySingleflight 带缓存的查询函数
func (r *reviewRepo) getDataBySingleflight(ctx context.Context, key string) ([]byte, error) {
	v, err, shared := g.Do(key, func() (any, error) {
		...
		//只有在缓存中没有key的时候，才去ES查询
		if errors.Is(err, redis.Nil) {
			//缓存没有key
			//查ES
			data, err := r.getDataFromES(ctx, key)
			r.log.Debugf("--> getDataFromES:data=%v, err=%v\n", data, err)
			if err == nil {
				//设置缓存
		...
		//查缓存失败了，直接返回错误，不继续向下传到压力（熔断）
		return nil, err
	})
	r.log.Debugf("--> getDataBySingleflight:v=%v, err=%v, shared=%v\n", v, err, shared)
	if err != nil {
		return nil, err
	}
	return v.([]byte), err
